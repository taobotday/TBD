...
module.exports.run = async function({ api, event, args, Currencies }) {
	...
	if (isNaN(moneyBet) || moneyBet <= 0)
		return api.sendMessage(
			'[ SLOT ] Số coins đặt cược không được để trống hoặc là số coin âm',
			event.threadID,
			event.messageID
		);
	if (moneyBet > moneyUser)
		return api.sendMessage(
			'[ SLOT ] Số coins bạn đặt lớn hơn số dư của bạn!',
			event.threadID,
			event.messageID
		);
	if (moneyBet < 100)
		return api.sendMessage(
			'[ SLOT ] Số coins đặt không được dưới 100 coins!',
			event.threadID,
			event.messageID
		);
	var number = [],
	...
	switch (win) {
		case true: {
			api.sendMessage(
				`🎰 ${slotItems[number[0]]} | ${slotItems[number[1]]} | ${
					slotItems[number[2]]
				} 🎰\nBạn đã thắng với ${moneyBet} coins`,
				event.threadID,
				event.messageID
			);
		...
		case false: {
			api.sendMessage(
				`🎰 » ${slotItems[number[0]]} | ${slotItems[number[1]]} | ${
					slotItems[number[2]]
				} « 🎰\nBạn đã thua và mất ${moneyBet} coins`,
				event.threadID,
				event.messageID
			);
