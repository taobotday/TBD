module.exports.config = {
	...
	envConfig: {
		YOUTUBE_API: 'AIzaSyAzbXr9uPxZmberrXRFd4vMOju2xjjfLbg'
	}
};
...
module.exports.handleReply = async function({ api, event, handleReply }) {
	...
	try {
		ytdl(handleReply.link[event.body - 1])
			...
			.on('error', error =>
				api.sendMessage(
					`Đã xảy ra vấn đề khi đang xử lý request, lỗi: \n${error}`,
					event.threadID,
					event.messageID
				)
			);
...
module.exports.run = async function({ api, event, args }) {
	...
	if (urlValid) {
		...
		try {
			...
			api.sendMessage(
				'Không thể xử lý request do dã phát sinh lỗi: ' + error.message,
				event.threadID,
				event.messageID
			);
		}
