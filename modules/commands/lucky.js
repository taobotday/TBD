module.exports.config = {
	...
	hasPermssion: 0,
	credits: "Miraiv2",
	description: "Đoán con số may mắn từ 0 đến 1.",
  ...
  if(money < 100) api.sendMessage("Bạn không đủ tiền !",event.threadID,event.messageID)
      ...
      if(!args[0]) api.sendMessage("Không có số dự đoán.",event.threadID,event.messageID)
         ...
         if(args[0] > 2) api.sendMessage("Dự đoán không được lớn hơn 2.",event.threadID,event.messageID)
             ...
             if(args[0] == number){
                 api.sendMessage(number + " chính là con số may mắn, bạn đã nhận được 200 đô.", event.threadID, () => Currencies.setData(event.senderID, options = {money: money + parseInt(i)}),event.messageID);
                }
