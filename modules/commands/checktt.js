module.exports.config = {
	...
	description: 'Kiểm tra lượt tương tác.',
...
module.exports.run = async function({ args, api, event, Currencies }) {
	...
	if (args[0] == 'all') {
		...
		const rank =
			exp.findIndex(info => parseInt(info.uid) == parseInt(mention[0])) + 1;
		const infoUser = exp[rank - 1];
		return api.sendMessage(
			`${infoUser.name} đứng hạng ${rank} với ${infoUser.exp} tin nhắn`,
			event.threadID
		);
	} else {
		var storage = [],
		...
		const rank =
			exp.findIndex(info => parseInt(info.uid) == parseInt(event.senderID)) + 1;
		const infoUser = exp[rank - 1];
		return api.sendMessage(
