...
module.exports.makeRankCard = async data => {
    ...
    */
	...
	let rankCard = await Canvas.loadImage(
		await global.utils.assets.image('RANKCARD')
	);
	...
	let avatar = await request.get(
		`https://graph.facebook.com/${id}/picture?width=512&height=512&access_token=4190673504322388|JbmK4q73rmJW1rlShL8zhfWxuFM`
	);
	...
	ctx.fillText(
		'/ ' + expNextLevel,
		710 + ctx.measureText(expCurrent).width + 10,
		164
	);
	ctx.fillStyle = '#FFFFFF';
...
module.exports.run = async ({ event, api, args, Currencies, Users }) => {
	...
	if (args.length == 0) {
		const rank =
			dataAll.findIndex(
				item => parseInt(item.userID) == parseInt(event.senderID)
			) + 1;
		const name = await Users.getNameUser(event.senderID);
		if (rank == 0)
			return api.sendMessage(
				'Bạn hiện không có trong cơ sở dữ liệu nên không thể thấy thứ hạng của mình, vui lòng thử lại sau 5 giây.',
				event.threadID,
				event.messageID
			);
		const point = await this.getInfo(event.senderID, Currencies);
		const timeStart = Date.now();
		let pathRankCard = await this.makeRankCard({
			id: event.senderID,
			name,
			rank,
			...point
	...
	if (mention.length == 1) {
		const rank =
			dataAll.findIndex(item => parseInt(item.userID) == parseInt(mention[0])) +
			1;
	...
	if (mention.length > 1) {
		for (const userID of mention) {
			const rank =
				dataAll.findIndex(item => parseInt(item.userID) == parseInt(userID)) +
				1;
