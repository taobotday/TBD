module.exports.config = {
	...
	hasPermssion: 2,
	credits: 'Miraiv2',
	description: 'Hiển thị tin nhắn sau khi bị gỡ.',
	...
	if (event.type == 'message_unsend') {
		...
		if (getMsg.attachment[0] == undefined)
			return api.sendMessage(
				`${name} đã gỡ 1 tin nhắn\nNội dung: ${getMsg.msgBody}`,
				threadID
			);
		else {
			let num = 0;
			let msg = {
				body: `${name} vừa gỡ ${getMsg.attachment.length} tệp đính kèm.${
...
module.exports.run = async function({ api, event, Threads }) {
	...
	return api.sendMessage(
		`Đã ${data['resend'] == true ? 'bật' : 'tắt'} resend thành công!`,
		threadID,
		messageID
	);
};
