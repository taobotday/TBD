...
module.exports.onLoad = async () => {
	...
	if (!existsSync(dirMaterial + 'canvas'))
		mkdirSync(dirMaterial, { recursive: true });
	if (!existsSync(path))
		await downloadFile('https://git.meewmeew.info/data/slap.png', path);
};
async function makeImage({ one, two }) {
	...
	let circleOne = await jimp.read(
		await circle(Buffer.from(avatarOne, 'utf-8'))
	);
	let circleTwo = await jimp.read(
		await circle(Buffer.from(avatarTwo, 'utf-8'))
	);
	slap_image
		.composite(circleOne.resize(150, 150), 745, 25)
...
module.exports.run = async function({ event, api, args }) {
	...
	if (!mention[0])
		return api.sendMessage('Vui lòng tags 1 người.', threadID, messageID);
	else {
		var one = senderID,
			two = mention[0];
		return makeImage({ one, two }).then(path =>
			api.sendMessage(
				{ body: 'CÚ ĐẤM YÊU THƯƠNG ❤', attachment: fs.createReadStream(path) },
				threadID,
				() => fs.unlinkSync(path),
				messageID
			)
