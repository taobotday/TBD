...
module.exports.run = async function ({ event, api, args }) {
    ...
    if (!mention[0]) return api.sendMessage("Vui lòng tags 1 người.", threadID, messageID);
    else {
        var one = senderID, two = mention[0];
        return makeImage({ one, two }).then(path => api.sendMessage({ body: "ĂN ĐẤM NÈEEE", attachment: fs.createReadStream(path) }, threadID, () => fs.unlinkSync(path), messageID));
    }
}
